...
const TimePerCardChart = ({ timePerCard, passfail, width = 350, height = 150, margin = { x: 20, y: 25 }, iconSize = 2 }: ChartProps) => {
    const data = makeTimeIndexTuple(timePerCard);

    const xScale = data && scaleLinear<number>({
        domain: extent(data, getX) as [number, number],
        range: [1.5 * margin.x, width + margin.x / 2]
    })

    const yScale = data && scaleLinear<number>({
        ...
        <>
            { data && xScale && yScale &&
                <section className="TimePerCard">
                    <div className="TimePerCard__header">
                        ...
                        >
                            ...
                            {data && data.map((d, i) =>
                                <circle
                                    ...
                                    cx={xScale(getX(d))}
                                    cy={yScale(getY(d))}
                                    fill={passfail[i] === 'pass' ? 'seagreen' : 'orangered'}
                                />
                            )}
