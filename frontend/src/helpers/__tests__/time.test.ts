...
describe('dayjs', () => {
    ...
    test('calculates diff between two dates properly', () => {
        expect(dayjs(now).diff(dayjs(new Date("March 21, 2020")), 'minutes')).toBeGreaterThan(0)
    })
...
describe('dateDifference', () => {
    test('... calculates positive difference in minutes properly', () => {
        expect(
            dateDifference(now, march, 'minutes')
        ).toBeGreaterThan(0)
    })

    test('... calculates negative difference properly', () => {
        expect(
            dateDifference(march, now, 'minutes')
        ).toBeLessThan(0)
    })

    test('... returns 0 if two supplied dates are equal', () => {
        expect(
            dateDifference(now, now, 'minutes')
        ).toBe(0)
    })
})
...
describe('humanizedDateDifference', () => {
    test("... returns 'a day' if 24 hours between two supplied dates", () => {
        ...
        expect(
            humanizedDateDifference(date1, date2)
        ).toBe('a day')
    })

    test("... returns 'a few seconds' if a few seconds between two supplied dates", () => {
        expect(
            humanizedDateDifference(new Date(), new Date(Date.now()-1000*2))
        ).toBe('a few seconds')
    })
})