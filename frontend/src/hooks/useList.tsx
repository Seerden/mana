...
import {
	...
	useMutateUpdateList,
	useQueryListsById,
} from "gql/hooks/list.query";
import { List } from "gql/codegen-output";
import {
	DeleteTermsVariables,
	useMutateDeleteTerms,
} from "gql/hooks/term.query";
...
function useList() {
	...
	const { mutate: mutateDeleteList, data: listDeleteResponse } =
		useMutateDeleteList(params.id);
	...
	const termsToDisplay = useMemo(() => {
		return filterTermsBySaturation(truncatedTerms)?.map((term) => term.element);
					...
					case "visible": // add all visible terms to termsToReview, as long as they're not already in there
						setTermsToReview((cur) =>
							Array.from(
								new Set([
									...cur,
									...filterTermsBySaturation(truncatedTerms).map((t) => t.term),
								])
							)
						);
						break;
