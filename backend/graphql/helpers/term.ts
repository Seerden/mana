...
export async function createTermDocuments(
    terms: Array<NewTermFromClient | NewListTerm>,
    options: any
) {
    ...
    termsToSave = termsToSave.map((term) => ({
        ...term,
...
export async function maybeDeleteTerms(ids: any[]) {
    ...
    if (ids.length > 0) {
        ...
        const terms = await TermModel.find({ _id: { $in: termIds } })
            .lean()
            .exec();
...
export async function appendListIdToTerms(
    listId: string | ObjectId,
    termIds: Array<string | ObjectId>
) {
    // const bulkOps = [];
