...
export async function updateListTerms(listId: string, remainingTermIds: [string]) {
    const remainingTermObjectIds = remainingTermIds.map((termId) => asObjectId(termId));
    const listObjectId = asObjectId(listId);
    ...
    const updatedList = await ListModel.findByIdAndUpdate(listObjectId, {
        $set: {
            ...
            terms: remainingTermObjectIds as ObjectId[],
        },
    });
...
export async function createListDocument(newList: NewListFromClient) {
    ...
    if (savedTerms.terms) {
        try {
            const termIds = savedTerms.terms.map(
                (term) => new mongoose.Types.ObjectId(term._id)
            );
            ...
            if (savedList) {
                ...
                const addedToUserBoolean = await addListToUser(
                    savedList._id,
                    savedList.owner
                );
...
export async function updateListDocument(
...
export async function maybeAddSessionToList(
    reviewSessionId: ObjectId,
    reviewSessionDate: Date,
    reviewDirection: "forwards" | "backwards",
    listIds: ObjectId[]
) {
    if (listIds.length == 1) {
